...
class Ndizi_Meta_Boxes {
	...
	public static function render_invoice_meta_box( $post ) {
		...
		if ( $project_id ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table name from $wpdb->prefix; ids are prepared; unbilled entries read from the custom table for the meta box.
	...
	public static function save_meta_boxes( $post_id ) {
		...
		if ( 'ndizi_invoice' === $post_type && isset( $_POST['ndizi_invoice_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ndizi_invoice_nonce'] ) ), 'ndizi_save_invoice' ) ) {
			...
			global $wpdb;
			$table_name = Ndizi_DB::get_table_name();
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Unlinking time entries from an invoice on the plugin's custom table.
			$wpdb->update(
				$table_name,
				array( 'invoice_id' => 0 ),
