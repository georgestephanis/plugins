...
- `includes/`
    ...
    - `class-ndizi-settings.php` — Settings page (module toggles driven by `get_module_registry()`, icon selection, lock date, webhook URLs, Google OAuth connect button, Stripe API keys); admin asset enqueueing; `admin_menu` registration (priority 9); Reports submenu (delegates page render to `Ndizi_Reports`); the **Time Entries** submenu (`admin.php?page=ndizi-time-entries`) and its `render_time_entries_page()` DataViews app; user profile billing/salary rate fields; Google OAuth2 callback handler; and `init_gantt()` / `register_gantt_admin_page()`. `register_dataviews_bundle()` registers the shared `build/vendor-dataviews.js`/`.css` as the `ndizi-dataviews` script + style handles (reading deps/version from `build/vendor-dataviews.asset.php`); the Time Entries app script lists `ndizi-dataviews` as a dependency so WordPress loads the shared bundle first, and the matching style is enqueued on that page. A **Client Portal** section lets an admin pick which page `Ndizi_Portal::get_portal_pages()` should treat as the default login destination (`ndizi_portal_page_id` option) when more than one published page contains the Client Portal block; calls into `Ndizi_Portal` here are not currently guarded by `is_module_active( 'portal' )`, unlike the invoicing/tracker/gantt/calendar sections on this same page.
    - `class-ndizi-meta-boxes.php` — Meta box registration and rendering for all six CPTs (`ndizi_client`, `ndizi_project`, `ndizi_task`, `ndizi_invoice`, `ndizi_contact`, `ndizi_time_off`) and the `save_post` handler that persists all custom fields. The Time Off meta box is only registered when the `time_off` module is active.
    - `class-ndizi-list-tables.php` — CPT list-table column additions (client, project, task, invoice), custom column rendering, the `pre_get_posts` handler that restricts list queries for team-member users, and a **Copy Portal Link** `post_row_actions` entry on the Clients screen that copies a client's login link (`Ndizi_Portal::get_client_portal_link()`) to the clipboard.
    ...
    - `class-ndizi-cpts.php` — Declares all post types (`ndizi_client`, `ndizi_project`, `ndizi_task`, `ndizi_invoice`, `ndizi_contact`, `ndizi_time_off`) and registers their REST metadata schemas. The `ndizi_invoice` CPT is only registered when the `invoicing` module is active; likewise the `ndizi_time_off` CPT and its meta registration are only registered when the `time_off` module is active. The `ndizi_time_off` CPT stores client-submitted time-off/absence requests with meta `_ndizi_time_off_start_date`, `_ndizi_time_off_end_date`, `_ndizi_time_off_type`, `_ndizi_time_off_status`, and `_ndizi_time_off_client_id`.
    ...
    - `class-ndizi-portal.php` — Loaded when the `portal` module is active. Handles shortcode execution, passwordless client token auth (`?ndizi_token=...`), client session cookies, discussion boards (filtered WordPress comments), file attachment uploads, time-off/absence request form submission (creates `ndizi_time_off` CPT posts, gated on `is_module_active( 'time_off' )`), and Stripe "Pay Online" button rendering for unpaid invoices when `ndizi_stripe_publishable_key` is set. Passes `rest_url` to all enqueued portal scripts. The shortcode and block both accept `enableTaskSubmission` (default `true`) and `enableTimeOff` (default `false`) attributes to show/hide those forms per instance. Tracks which published pages contain the Client Portal block via `save_post`/`deleted_post` hooks (`track_portal_page_on_save()` / `untrack_portal_page_on_delete()`), caching the result in the `ndizi_portal_pages` option; `get_portal_pages()` does a one-time `LIKE`-scoped search backfill for pre-existing content, and `get_client_portal_link()` resolves a client's login URL against the cached list or the `ndizi_portal_page_id` override set in Settings.
    ...
    - `class-ndizi-abilities.php` — Registers the WordPress Abilities API surface (`wp_register_ability()`) for agentic/MCP workflows, mirroring `class-ndizi-rest.php`'s data access under `ndizi/`-prefixed ability IDs instead of REST routes. Covers full CRUD for every relational entity — clients, projects, tasks, invoices, contacts, time off — plus read/update/delete for time entries (creation is covered by the existing `log-time-manual`/`start-timer`/`stop-timer` abilities, which delegate to `Ndizi_Time_Service` like their REST/AJAX/CLI counterparts). Every `permission_callback` reuses the same capability gate as the equivalent REST endpoint (e.g. invoice abilities call `Ndizi_REST::check_view_invoices_permission()` directly rather than re-implementing it) so an ability can never see or do more than the REST API already allows for that user. `create-*`/`update-*` abilities validate required fields and enum values against the same allowed value sets as the meta boxes/REST layer before writing. `delete-*` abilities run a dependency guard (`find_blocking_dependents()`/`dependents_error()`) that blocks deletion with a `WP_Error` if dependent records exist (e.g. a client with projects, a project with tasks or invoices) instead of silently orphaning data. Time-entry abilities reuse `Ndizi_DB` directly and enforce the same non-manager-scoped-to-self, lock-date, and approval-field rules as `class-ndizi-rest.php`. `_ndizi_client_auth_key` (the client portal login secret) is never exposed by any ability's output schema.
...
- `src/`
    ...
    - `block/` — The `ndizi/client-portal` editor block (`block.json`, `index.js` edit UI, `render.php` dynamic frontend render) wrapping the portal shortcode. `block.json` declares `enableTaskSubmission` (default `true`) and `enableTimeOff` (default `false`) boolean attributes; `index.js` exposes them as `ToggleControl`s in the block's `InspectorControls`.
- `templates/`
...
| `time_off` | (inline, gated in `class-ndizi-cpts.php` / `class-ndizi-meta-boxes.php` / `class-ndizi-portal.php`) | Time Off Requests CPT/admin screen, out-of-office portal form |
...
- `ndizi_client` — Contains client metadata (`_ndizi_client_website`, `_ndizi_client_address`, `_ndizi_client_status`, `_ndizi_client_auth_key`, `_ndizi_client_billing_mode`). `_ndizi_client_billing_mode` (`''`/`client`/`project`/`both`) overrides the global `ndizi_billing_mode` option for that client on new invoices; empty inherits the global default. Resolve the effective value via `Ndizi_Project_Management::get_billing_mode( $client_id = 0 )` rather than reading either value directly.
...
- `ndizi_invoice` — Linked to a client (`_ndizi_client_id`) and project (`_ndizi_project_id`), with `_ndizi_invoice_number`, `_ndizi_invoice_currency`, date (`_ndizi_invoice_date`), due date (`_ndizi_invoice_due_date`), total amount (`_ndizi_invoice_amount`), status (`_ndizi_invoice_status`), and array metas `_ndizi_invoice_line_items` and `_ndizi_invoice_payments`. Only registered when the `invoicing` module is active.
...
- `ndizi_time_off` — Client-submitted absence/time-off requests created from the portal. Meta: `_ndizi_time_off_start_date`, `_ndizi_time_off_end_date`, `_ndizi_time_off_type` (`vacation`, `sick_leave`, `personal`, `other`), `_ndizi_time_off_status` (`pending`/`approved`/`rejected`), `_ndizi_time_off_client_id`.
