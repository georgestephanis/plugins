...
- **Time Entries Management Screen**: A dedicated admin page (`admin.php?page=ndizi-time-entries`) built on the core [`@wordpress/dataviews`](https://www.npmjs.com/package/@wordpress/dataviews) component — a full-width, sortable, filterable React table of all entries with inline create/edit/delete. Project and task are stacked in one column (sorting groups by project, then task), start/end collapse into a single Date column, the Description column wraps and auto-expands to fill available width, and Duration/Billable/Status stay compact. Managers can filter by project, user, billable status, and approval status — and **bulk-approve or unapprove** multiple entries via row-selection checkboxes; team members see their own entries. DataViews is shipped as a shared, separately-built bundle (see [Working with Assets](#working-with-assets)).
